...
function getRandom(max) {
  let r = Math.random(); // 0.0xxx ~ 0.9xxx 즉 1보다 작은 수
...
function getResult(n1, n2) {
  ...
  // 이 함수를 어떠한 UI가 호출할지 예측할 수가 없으므로,
  // js와 관련된 모든 페이지에서 이 함수를 재사용하기위해서는 함수는 중립적일수록 좋다..
  // 따라서 반환값을 적극 활용하면 좋다
  // 즉 함수를 호출한者가 무엇을 할지 예측할 수 없기 때문에,
  // 반환된 데이터를 가지고 무엇을할지에 대한 결정은 호출자에게 맡기자
  return result;
}
...
/*----------------------------------------------
매개변수1 : 조사대상 배열
매개변수2 : 어떤 값을 조사할지 해당 값
반환값 : 중복이 발견되면 true, 아니면 false
----------------------------------------------*/
function checkDuplicated(array, n) {
  let result = false; // 중복이 발견되지 않았음..
  
  // 조사 : 어디에 중복된 데이터가 있는지 알수없으므로,
  //        모든 배열 요소를 다 조사하자
  for(let i=0; i<array.length; i++) {
    if(array[i] == n) { // 드뎌 중복 발견!!
      result = true;
      break;
    }
  }
